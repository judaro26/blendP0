...
exports.handler = async function(event) {
  ...
  let body;
  try {
    log.push('Attempting to parse event body...');
    body = JSON.parse(event.body);
    log.push(`Successfully parsed event body. Body keys: ${Object.keys(body).join(', ')}`);
  } catch (err) {
  ...
  try {
    ...
    log.push(`Starting loop to create Freshdesk tickets...`);
    for (const [depKey, data] of Object.entries(matchedData)) {
      try {
        log.push(`Processing deployment '${depKey}'...`);
        ...
        let ticketForm;
        try {
          ticketForm = new FormData();
        } catch (formDataError) {
          log.push(`ERROR: Failed to create FormData instance: ${formDataError.message}`);
          throw formDataError;
        }
        ...
        log.push(`Sending request to Freshdesk API for deployment '${depKey}'...`);
        ...
        log.push(`Freshdesk API response received. Status: ${ticketResp.status}`);
        const ticketResult = await ticketResp.json();
        log.push(`Freshdesk API response body: ${JSON.stringify(ticketResult)}`);
        ...
        log.push(`INNER CATCH: An error occurred while processing deployment '${depKey}': ${innerErr.message}`);
        freshdeskResults.push({ deployment: depKey, status: 'Failed', ticket_id: null, error_details: { message: innerErr.message } });
        continue;
      }
    }
